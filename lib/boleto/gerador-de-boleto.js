	...
	GeradorDeBoleto.prototype.gerarPDF = function(args) {
		return new Promise((resolve)=> {
			...
			boletos.forEach(function escreveOsDadosDoBoleto(boleto, indice) {
				...
				const nomeSacadorAvalista = sacadorAvalista ? sacadorAvalista.getNome() : '';
				...
				if(modeloReciboPagador===3){ 
					...
					instrucoes.forEach(function(instrucao, indice) {
						...
						pdf.font('normal')
							...
							.text(instrucao, posX+args.paddingCampos, posY+args.paddingCampos, {
								lineBreak: false,
								width: widthCampo-(args.paddingCampos*2),
				...
				instrucoes.forEach(function(instrucao, indice) {
					pdf.font('normal')
						.fontSize(args.fontSizeTextos*.966)
						.text(instrucao, posX+args.paddingCampos, posY+args.fontSizeTitulos+(args.paddingCampos*2)+((args.fontSizeTextos+args.paddingCampos)*indice), {
							lineBreak: true,
							width: widthCampo-(args.paddingCampos*2),
							align: 'left'
						});
				...
				args.creditos && pdf.font('italico')
					.fontSize(8)
					.text(args.creditos, args.marginLeft, posY, {
						width: 560,
						align: 'center'
					});
